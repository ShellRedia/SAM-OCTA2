...
class PredictManager:
    def __init__(self):
        ...
        weight_path = "checkpoints/rv_predictor_500.pth"
    ...
    def predict(self):
        ...
        for samples, (sample_name, layer_name) in self.dataloader:
            ...
            rv_gt_dir = "datasets/OCTA500/2D/{}/GT_LargeVessel".format(fov)
            ...
            overlay_mask = self.overlay(sample_image * 2, self.to_color(pred_mask, (0,1,0)))
...
if __name__=="__main__":
    pm = PredictManager()
    # prediction_mask = cv2.imread("prediction_mask.png", cv2.IMREAD_GRAYSCALE)
    # smoothed_mask = pm.smooth_mask(prediction_mask)
    # cv2.imwrite("smoothed_mask.png", np.concatenate([prediction_mask, smoothed_mask], axis=1))
    pm.predict()